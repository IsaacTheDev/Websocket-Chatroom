...
MSG_socket.onopen = function (event) {
  ...
  MSG_socket.send(JSON.stringify(data));
  // Uncomment for Auth Messages
  // Toastify({
...
MSG_socket.onmessage = function (event) {
  ...
  if (data["type"] == "join" && data["uid"] == uid) {
    // Uncomment for Auth Messages
    // Toastify({
    //   text: "Authenticated with server",
    //   duration: 3000,
    //   style: {
...
MSG_socket.onclose = function (event) {
  if (event.wasClean) {
    if (parseInt(event.reason) == 1) {
      ...
      Toastify({
        text: "Connection Closed: " + event.reason,
        duration: 3000,
        style: {
          background: "linear-gradient(to right, #EA4C46, #DC1C13)",
        },
...
MSG_socket.onerror = function (error) {
  Toastify({
    text: "Error Unknown",
    duration: 3000,
    style: {
